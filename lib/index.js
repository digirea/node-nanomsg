...
Socket.prototype._retrySend = function (msg, flags, count) {
	var result = -1;
	if (count < 10) {
		nn.Setopt(this.binding, 0, 4, this.timeout); // NN_SOL_SOCKET, NN_SNDTIMEO, default 1000ms
		result = nn.Send(this.binding, msg, flags);
		if (result == -11) {
			console.warn("retry send!");
			return this._retrySend(msg, flags, count + 1);
		} else {
			return result;
		}
	}
...
Socket.prototype._send = function (msg, flags) {
    ...
    return this._protect(this._retrySend(msg, flags, 0), function () {
